...
internal class Task5TheMostFrequentTripDurationPeriodTest {
    ...
    private fun testDurationPeriod(expected: Set<IntRange?>, tp: TaxiPark) {
        ...
        if (expected.size <= 1) {
            assertThat(actual).describedAs(message + tp.display()).isEqualTo(expected.firstOrNull())
        } else {
            assertThat(actual?.let { it in expected } ?: expected.isEmpty())
                .describedAs(tp.display() +
